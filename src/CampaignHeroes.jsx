...
export default function CampaignHeroes({ onBack }) {
  ...
  async function loadStats() {
    try {
      ...
      const deckStatsData = await AirtableService.getDeckStats();
      setDeckStats(deckStatsData.sort((a, b) => b.winRate - a.winRate));
      
      const totalVictories = deckStatsData.reduce((sum, deck) => sum + deck.wins, 0);
      const avgWinRate = deckStatsData.length > 0
        ? deckStatsData.reduce((sum, deck) => sum + deck.winRate, 0) / deckStatsData.length
        : 0;
      
      setOverallStats({
        totalDecks: deckStatsData.length,
        totalVictories,
